...
export function useFavoriteCard({card_or_fav_id,name,type}:{card_or_fav_id: string, name: string, type: "add" | "remove"}) {
	return useQuery(["PostFavoriteCard", card_or_fav_id, name, type], () => FavoriteCard(card_or_fav_id, name, type), { staleTime: 0, enabled: false })
}
