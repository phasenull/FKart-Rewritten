...
interface KentKartAuthStore {
	...
	fetchAccessToken: (
		...
		isOTP?: boolean
	) => Promise<[false, string | undefined] | [string]>;
	...
	legacy_login: (args: {
		...
		password: string;
		auth_type: AuthTypes;
	}) => Promise<[true] | [false, string | undefined]>;
	otp_login: (args:{
		type:AuthTypes,
		username:string,
...
const storage: StateStorage = {
	getItem: async (name: string): Promise<string | null> => {
		const data =
			(await ApplicationConfig.database.get(`zustand.kentkart.${name}`)) ||
			null;
		return data;
	},
...
export const useKentKartAuthStore = createWithEqualityFn<KentKartAuthStore>()(
	persist(
		(set, get) => ({
			user: undefined,
			region: undefined,
			...
			otp_login: async (args) => {
				const { type, username, one_time_code } = args;
				const region = get().region;
				if (!region) {
					Logger.error("KentKartAuthStore.otp_login", "region not set");
					throw new Error("region not set");
					return;
				}
				const url = `https://auth.kentkart.com/rl1/oauth/otp?${new URLSearchParams({
					region: region,
					authType: "004",
				})}`;
				console.log("Attempting OTP login", url);
				
			},
			legacy_login: async (args) => {
				const { auth_type, password, username } = args;
				console.log(
					`Attempting to login ${username} ${password.slice(
						0,
						3
					)}... ${auth_type}`
				);
				...
				await ApplicationConfig.database.set(
					"kentkart.one_time_code",
					one_time_code
				);
				const [access_token, access_token_error] = await get().fetchAccessToken(
					one_time_code,
					true
				);
				if (!access_token) {
			...
			fetchAccountInfo: async () => {
				...
				if (!access_token)
					return [false, "access token not found, please log in"];
				const [data, error] = await API.fetchProfile({
					access_token: access_token,
					auth_type: auth_type,
				...
				set({ user: data });
				Logger.info(
					"KentKartAuthStore.tsx.fetchAccountInfo",
					"account info fetch successfull"
				);
				return [data];
			},
			register: async (args) => {},
			logout: () => {
				Logger.info(
					"KentKartAuthStore.tsx.logout",
					`Logging out from account ${get().user?.name}`
				);
				set({
					user: undefined,
					credentials: { access_token: undefined, refresh_token: undefined },
				});
			},
		}),
		{
