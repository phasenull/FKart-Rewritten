...
export default function RouteList(props: {
	...
	const busListFromMemo = useMemo(() => {
		return data.routeList?.filter(
			(item: BasicRouteInformation) => item.routeType === "3"
		)
	}, [])
	const tramListFromMemo = useMemo(() => {
		return data.routeList?.filter(
			(item: BasicRouteInformation) => item.routeType === "0"
		)
	}, [])
	const ferryListFromMemo = useMemo(() => {
		return data.routeList?.filter(
			(item: BasicRouteInformation) => item.routeType === "4"
		)
	}, [])
	...
	return useMemo(
		() => (
			<FlatList
				...
				ListEmptyComponent={() => <Text className="text-xl mx-auto mt-5 font-bold" style={{color:Application.styles.secondary}}>No data found!</Text>}
