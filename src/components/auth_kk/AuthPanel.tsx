...
export default function AuthPanel(props: AuthPanelProps) {
	...
	const login = useKentKartAuthStore((state) => state.legacy_login);
	...
	return (
		<View
			...
			className="py-4 flex-1 flex-col items-center justify-evenly w-1/2"
		>
			<KeyboardAvoidingView
				behavior={Platform.OS === "ios" ? "padding" : "padding"}
				className="mb-4 mt-12 flex-col mx-auto justify-center w-64"
			>
				<Text
				...
				>
					{panel_type === 0
						? translations.screens.auth.signin.hello
						: translations.screens.auth.signup.hello}
				</Text>
				{/* username */}
				<TextInput
					...
					placeholder={
						inputType === AuthTypes.email
							? translations.input_fields.email
							: translations.input_fields.phone
					}
					className="mt-8 border-2 w-full h-16 border-slate-200 rounded-[10px] px-8 py-2 shadow-2xl"
					value={
						input_fields[inputType === AuthTypes.email ? "email" : "tel"] || ""
					}
					onChangeText={(text) => {
						setInputFields({
							...input_fields,
							[inputType === AuthTypes.email ? "email" : "tel"]: text,
						});
					}}
				/>
				{/* password */}
				...
				<View className="flex flex-row items-center align-middle justify-end -mt-2">
					<Text style={{ color: theme.secondaryDark }}>
						{translations.input_fields.use_phone}
					</Text>
					<Switch
							...
							switch (inputType) {
								case AuthTypes.phone:
									if (panel_type === 0) {
										...
										const [success, errorstring] = await login({
											password: input_fields.password,
											username: input_fields.tel,
					...
					>
						...
						>
							<Text
								...
								className="text-2xl text-center font-bold pl-5 px-2 py-3"
							>
								{panel_type === 0 ? translations.signin : translations.signup}
								{/* {"    ➜"} */}
							</Text>
			...
			<SwitchAuthPage
				style={{ marginBottom: is_keyboard_open ? 4 * 4 : 12 * 4 }}
