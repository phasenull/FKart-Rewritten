...
export default function LogsView() {
	...
	const { isFetching, data, isRefetching,isError,error } = useQuery(["getLogs"], () => getLogsAsync(userManager.accessToken))
	if (isFetching || isRefetching) {
		return <CustomLoadingIndicator size={12*4} />
	}
	if (isError) {
		return <View style={{backgroundColor:theme.white,elevation:10,borderRadius:16,paddingHorizontal:4*4,paddingVertical:2*4,width:"80%",maxWidth:80*4,minWidth:40*4}}>
			<SecondaryText style={{color:theme.error}}>
				{(error as any)?.response?.data?.result?.error || "unknown error"}
			</SecondaryText>
		</View>
	}
	return <View style={{backgroundColor:theme.white,elevation:10,borderRadius:16,paddingHorizontal:4*4,paddingVertical:2*4,width:"80%",maxWidth:80*4,minWidth:40*4}}>
		{data?.data.logs.map((logObject)=><LogTouchable key={logObject.id} log={logObject} />)}
	</View>
}
